...
const CreateItineraryForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/addItinerary', formData);
      setMessage(response.data.msg);
      setError(''); // Clear any previous error
    } catch (err) {
      setError(err.response?.data?.error || 'An error occurred while creating the itinerary.');
