...
const UpdateItineraryForm = ({ itineraryID }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:5000/updateItinerary/${itineraryID}`, formData);
      setMessage('Itinerary updated successfully!');
      setError(''); // Clear any previous error
    } catch (err) {
      setError(err.response?.data?.error || 'An error occurred while updating the itinerary.');
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input type="text" name="locations" value={formData.locations} onChange={handleChange} required />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" name="availability_dates" value={formData.availability_dates} onChange={handleChange} />
        </div>
        <div>
