...
function AdminFrontend() {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/getadminbyId/${adminId}`);
        setAdminUsername(response.data.username);
      } catch (error) {
        console.error("Error fetching admin data:", error);
      }
  ...
  const fetchTags = async () => {
    ...
    setTagError(null);
    try {
      const response = await axios.get("http://localhost:3000/getPreferancetag");
      setTags(response.data);
    } catch (error) {
      setTagError("Error retrieving tags");
    } finally {
      ...
      <div className="admin-container">
        ...
        <aside className="admin-sidebar">
          <ul>
            <li onClick={() => setCurrentSection("dashboard")}>🏠 Dashboard</li>
            <li onClick={() => setCurrentSection("tags")}>🏷️ Tags</li>
            <li onClick={() => setCurrentSection("categories")}>📂 Categories</li>
            <li onClick={() => setCurrentSection("products")}>🛒 Products</li>
            <li onClick={() => setCurrentSection("complaints")}>⚠️ Complaints</li>
            <li onClick={() => setCurrentSection("salesReport")}>📊 Sales Report</li>
            <li onClick={() => setCurrentSection("userStatistics")}>📈 User Statistics</li>
          </ul>
        </aside>
