...
const CreateTouristItineraryForm = ({ id }) => {
  // Accept id as a prop
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:3000/createTouristItineraries/${id}", {
        ...itineraryData,
      ...
      const errorMessage =
        error.response?.data?.error || "Error creating itinerary"; // Fallback error message
      alert(errorMessage);
    }
  };
    ...
    <form onSubmit={handleSubmit}>
        ...
        <input
          ...
          onChange={(e) =>
            setItineraryData({
              ...itineraryData,
              availability_dates: {
                ...itineraryData.availability_dates,
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setItineraryData({ ...itineraryData, activities: e.target.value })
          }
        />
      </div>
